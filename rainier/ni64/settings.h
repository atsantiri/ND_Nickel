...
const int g_nAMass = 65; // proton + neutron number
const int g_nDisLvlMax = 62; // only trust level scheme to here, sets ECrit

const bool g_bIsEvenA = !(g_nAMass % 2);
const int g_nDisLvlGamMax = 62; // max #gammas read for a discrete lvl from file
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLD_CTM
  const double g_dTemp =  1.014; // MeV ROBIN for 91Nb= 0.908 
  const double g_dE0   = -0.720; // MeV ROBIN for 91Nb= -0.606 
  ...
  #ifdef bLD_BSFG
  const double g_dE1 = -0.097; // MeV, excitation energy shift from nndc pairing gap
  ...
  #ifdef bLDaConst
  const double g_dLDa = 7.721; // MeV^-1 aka "LD parameter a" 
  #endif
  #ifdef bLDaEx
...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  const double g_adSigE1[] = { 112.96055502699117, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 18.033211938220454, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = { 6.517378503413406, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
  ...
  const double g_adSigM1[] = { 3.6135622074782248, 0.00}; // mb magnitude
  const double g_adEneM1[] = { 10.197164010108366, 0.01}; // MeV centroid energy, non-zero
...
// similar to (n,g)
const double g_dExIMax = 7.3; // MeV, Ei - "capture state energy"
...
const double g_adExI[] = {10.5}; // MeV
const double g_adSpI[]  = {1.5}; // hbar
const double g_anParI[] = {1}; // Pi
const double g_adBRI[]   = {1}; // Branching Ratio: has to sum up to 1.0
#endif
