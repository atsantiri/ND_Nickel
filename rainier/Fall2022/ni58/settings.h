...
const int g_nAMass = 59; // proton + neutron number
const int g_nDisLvlMax = 39; // only trust level scheme to here, sets ECrit

const bool g_bIsEvenA = !(g_nAMass % 2);
const int g_nDisLvlGamMax = 39; // max #gammas read for a discrete lvl from file
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLD_CTM
  const double g_dTemp =  1.345; // MeV ROBIN for 91Nb= 0.908 
  const double g_dE0   = -0.932; // MeV ROBIN for 91Nb= -0.606 
  ...
  #ifdef bLD_BSFG
  const double g_dE1 = -0.309; // MeV, excitation energy shift from nndc pairing gap
  ...
  #ifdef bLDaConst
  const double g_dLDa = 5.097; // MeV^-1 aka "LD parameter a" 
  #endif
  #ifdef bLDaEx
...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  const double g_adSigE1[] = { 99.22686900435892, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 18.45498642674014, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = { 6.8116225177202745, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
  ...
  const double g_adSigM1[] = { 4.093169613798932, 0.00}; // mb magnitude
  const double g_adEneM1[] = { 10.531733302611595, 0.01}; // MeV centroid energy, non-zero
