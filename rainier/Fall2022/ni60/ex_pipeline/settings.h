...
const int g_nAMass = 61; // proton + neutron number
const int g_nDisLvlMax = 46; // only trust level scheme to here, sets ECrit

const bool g_bIsEvenA = !(g_nAMass % 2);
const int g_nDisLvlGamMax = 46; // max #gammas read for a discrete lvl from file
                                // suggestion: g_nDisLvlMax

#define buse_nondefault_levels_file
#ifdef buse_nondefault_levels_file
const TString slevels_file = "mylevels_z029.dat"; // might e.g. add missing half-life
...
#else
const double g_dConESpac = 0.05; // MeV; wont matter if forcing bin number
int g_nConEBin;
#endif // bForceBinNum
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLD_CTM
  const double g_dTemp =  1.195; // MeV ROBIN for 91Nb= 0.908 
  const double g_dE0   = -0.854; // MeV ROBIN for 91Nb= -0.606 
  ...
  #ifdef bLD_BSFG
  const double g_dE1 = -0.231; // MeV, excitation energy shift from nndc pairing gap
  //const double g_dDeuPair = -0.324; // MeV; can get from ROBIN: Pa_prime
  ...
  #ifdef bLDaConst
  const double g_dLDa = 6.103; // MeV^-1 aka "LD parameter a" 
  #endif
  #ifdef bLDaEx
...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  const double g_adSigE1[] = { 103.94108732377231, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 18.308574536786374, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = { 6.708779298741742, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
  ...
  const double g_adSigM1[] = { 3.9328100634961745, 0.00}; // mb magnitude
  const double g_adEneM1[] = { 10.415351032384018, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamM1[] = { 4.0, 0.00}; // MeV GDR width
  #ifdef bM1StrUpbend // soft pole behavior: C * exp(-A * Eg)
  const double g_dUpbendM1Const = 1e-6; // C
...
// similar to (n,g)
const double g_dExIMax = 7.12; // MeV, Ei - "capture state energy"
...
const double g_adExI[] = {mple}; // MeV
const double g_adSpI[]  = {mple};//,0.5,1.5}; // hbar
const double g_anParI[] = {mple};//,0,0}; // Pi 1 for +, 0 for -
const double g_adBRI[]   = {1};//0.5,0.25,0.25}; // Branching Ratio: has to sum up to 1.0
#endif
...
// dont exceed with init excitations - gaus might sa6.000 higher than expected
