...
const int g_nAMass = 62; // proton + neutron number
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLD_CTM
  const double g_dTemp =  1.145; // MeV ROBIN for 91Nb= 0.908 
  const double g_dE0   = -2.224; // MeV ROBIN for 91Nb= -0.606 
  ...
  #ifdef bLD_BSFG
  const double g_dE1 = -1.601; // MeV, excitation energy shift from nndc pairing gap
  ...
  #ifdef bLDaConst
  const double g_dLDa = 6.492; // MeV^-1 aka "LD parameter a" 
  #endif
  #ifdef bLDaEx
...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  const double g_adSigE1[] = { 106.24532119265315, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 18.237631335706, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = { 6.659215262345964, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
  ...
  const double g_adSigM1[] = { 3.8525144026473512, 0.00}; // mb magnitude
  const double g_adEneM1[] = { 10.35905073795356, 0.01}; // MeV centroid energy, non-zero
