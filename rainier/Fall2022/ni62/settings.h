...
const int g_nDisLvlMax = 60; // only trust level scheme to here, sets ECrit

const bool g_bIsEvenA = !(g_nAMass % 2);
const int g_nDisLvlGamMax = 60; // max #gammas read for a discrete lvl from file
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLD_CTM
  const double g_dTemp =  1.089; // MeV ROBIN for 91Nb= 0.908 
  const double g_dE0   = -0.747; // MeV ROBIN for 91Nb= -0.606 
  ...
  #ifdef bLD_BSFG
  const double g_dE1 = -0.124; // MeV, excitation energy shift from nndc pairing gap
  ...
  #ifdef bLDaConst
  const double g_dLDa = 6.984; // MeV^-1 aka "LD parameter a" 
  #endif
  #ifdef bLDaEx
...
#include "GSFTable.dat" // just placeholder values in this file for now
  ...
  const double g_adSigE1[] = { 108.51590412868522, 0.00}; // mb magnitude
  const double g_adEneE1[] = { 18.168123577581937, 0.01}; // MeV centroid energy, non-zero
  const double g_adGamE1[] = { 6.610823948218521, 0.00}; // MeV GDR width
  //                                  ^^^^ for a 2nd resonance
  ...
  const double g_adSigM1[] = { 3.7724270141613045, 0.00}; // mb magnitude
  const double g_adEneM1[] = { 10.303948362098437, 0.01}; // MeV centroid energy, non-zero
