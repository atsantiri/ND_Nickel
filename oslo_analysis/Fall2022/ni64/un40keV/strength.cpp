{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,  13.156,10,3.460e-09,4.500e-04);
	ifstream strengthfile("strength.nrm");
	float strength[86],strengtherr[86],energyerr[86];
	float energy[851],trans[851];
	int i = 0;
   float a0 =  -0.7840;
   float a1 =   0.1600;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-85] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(86,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    2.531,1.350e-04,"^{xx}Yy");
	c1->Update();
