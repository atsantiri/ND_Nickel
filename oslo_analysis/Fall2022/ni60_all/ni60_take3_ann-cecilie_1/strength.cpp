{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,  10.596,10,4.164e-09,8.980e-06);
	ifstream strengthfile("strength.nrm");
	float strength[93],strengtherr[93],energyerr[93];
	float energy[921],trans[921];
	int i = 0;
   float a0 =  -0.8240;
   float a1 =   0.1200;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-92] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(93,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    2.019,2.694e-06,"^{xx}Yy");
	c1->Update();
