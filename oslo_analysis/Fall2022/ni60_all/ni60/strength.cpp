{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,  10.016,10,9.384e-11,6.121e-06);
	ifstream strengthfile("strength.nrm");
	float strength[66],strengtherr[66],energyerr[66];
	float energy[651],trans[651];
	int i = 0;
   float a0 =  -0.7240;
   float a1 =   0.1600;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-65] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(66,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.903,1.836e-06,"^{xx}Yy");
	c1->Update();
