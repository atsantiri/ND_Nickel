{
	...
	TH2F *h = new TH2F("h"," ",10,0.0,  10.336,10,1.927e-10,4.237e-07);
	ifstream strengthfile("strength.nrm");
	float strength[68],strengtherr[68],energyerr[68];
	float energy[671],trans[671];
	int i = 0;
   float a0 =  -0.7240;
   float a1 =   0.1600;
	float x;	
	while(strengthfile){
		...
		else{strengtherr[i-67] = x;}
		i++;
	}
	TGraphErrors *strengthexp = new TGraphErrors(68,energy,strength,energyerr,strengtherr);
    i = 0;
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.967,1.271e-07,"^{xx}Yy");
	c1->Update();
