...
//////////////////////////////////////////////////////
{
	...
	// declarations
	ifstream  ifile1("fg.rsg"), ifile2("fgerr.rsg"), ifile3("fgteo.rsg");
	string line;
	string cal_dummy;
	string dim_dummy;
	...
	else cout << " Dimension on y axis is: " << dy-2 << " ch." << endl;	
	dy = dy-2;
    ...
    TH2D *h   = new TH2D("h",  " ",100,800,6400,100,0.001,0.31);
    TH2D *h_1 = new TH2D("h_1"," ",100,800,6400,100,0.001,0.31);
    TH2D *h2  = new TH2D("h2", " ",100,800,6400,100,0.001,0.31);
    TH2D *h_2 = new TH2D("h_2"," ",100,800,6400,100,0.001,0.31);
    TH2D *h_3 = new TH2D("h_3"," ",100,800,6400,100,0.001,0.31);
    ...
    text.SetTextFont(42);
    text.DrawLatex(2200,0.28,TString::Format("(a) E_{x} = %.1f MeV",matrix->GetYaxis()->GetBinCenter(bin1)*1e-3));
	
    TLegend *leg = new TLegend(0.5,0.6,0.85,0.78);
    leg->SetBorderSize(0);
    leg->SetFillColor(0);
    leg->SetTextFont(42);
    ...
    text.DrawLatex(2200,0.28,TString::Format("(b) E_{x} = %.1f MeV",matrix->GetYaxis()->GetBinCenter(bin2)*1e-3));
    ...
    text.DrawLatex(2200,0.28,TString::Format("(c) E_{x} = %.1f MeV",matrix->GetYaxis()->GetBinCenter(bin3)*1e-3));
    ...
    text.SetTextSize(0.08);
    text.DrawLatex(2200,0.28,TString::Format("(d) E_{x} = %.1f MeV",matrix->GetYaxis()->GetBinCenter(bin4)*1e-3));
    ...
    graph5->Draw("P same");
    text.DrawLatex(2200,0.28,TString::Format("(e) E_{x} = %.1f MeV",matrix->GetYaxis()->GetBinCenter(bin5)*1e-3));
    ...
    graph6->Draw("P same");
    text.DrawLatex(2200,0.28,TString::Format("(f) E_{x} = %.1f MeV",matrix->GetYaxis()->GetBinCenter(bin6)*1e-3));


    c1->Update();
	
	// Print to pdf file
	c1->Print("does_it_work_51Ti_NSCL.pdf");
    c1->Print("does_it_work_51Ti_NSCL.eps");
